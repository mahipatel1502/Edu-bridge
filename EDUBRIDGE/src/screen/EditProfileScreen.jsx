...
const EditProfileScreen = () => {
  ...
  const { userData, onProfileUpdate } = route.params; // Function to update UI instantly
  ...
  const handleUpdate = async () => {
    try {
      ...
      // Save the updated data in AsyncStorage
      await AsyncStorage.setItem("user", JSON.stringify(result.updatedUser));

      // ✅ Update UI Instantly
      if (onProfileUpdate) {
        onProfileUpdate(result.updatedUser); // Pass updated data back to parent
      }
